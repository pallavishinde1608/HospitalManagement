...
<center>
			<input type='text' name='name' value='' id="s"
				...
				placeholder="search" onkeyup="searchByName()" /> </center> 
  ...
  for(DoctorModel d:list)
   {
	 %>
	   ...
	   <td><a href='updDoct?did=<%=d.getDid()%>&name=<%=d.getName()%>&contact=<%=d.getContact()%>&specialization=<%=d.getSpecialization()%>&email=<%=d.getEmail()%>&schedule=<%=d.getSchedule()%>&category=<%=d.getCategory()%>'><%=d.getDid() %></a></td>
